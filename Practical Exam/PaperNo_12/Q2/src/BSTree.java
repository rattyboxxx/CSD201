...
public class BSTree {
    ...
    */
    void insert(String xOwner, int xPrice, int xColor) {
        ...
        Node father = null;
        while (cu != null) {
            if (cu.getInfo().price == x.price) {
                return;
            }
            father = cu;
            if (cu.getInfo().price < x.price) {
                cu = cu.right;
            } else if (cu.getInfo().price > x.price) {
                cu = cu.left;
            }
        }
        assert (father != null);
        if (father.getInfo().price > x.price) {
            father.left = node;
        } else {
            father.right = node;
        }
    ...
    public void breadth3(Node p, RandomAccessFile f) throws Exception {
        if(p == null) 
            return;
        Queue q = new Queue();
        q.enqueue(p);
